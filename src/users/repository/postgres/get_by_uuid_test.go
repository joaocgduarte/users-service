...
func Test_GetByUuid_ErrorPreparingContext(t *testing.T) {
	...
	user, err := repo.GetByUUID(context.TODO(), uuid.New())
	assert.Nil(t, user)
	assert.Error(t, err)
	assert.Equal(t, err.Error(), "boom")
}

func Test_GetByUuid_TimeoutReached(t *testing.T) {
	db, mock, err := sqlmock.New()
	assert.Nil(t, err)
	defer db.Close()
	...
	repo := PostgresRepository{db}
	user, err := repo.GetByUUID(ctx, userId)
	assert.Nil(t, user)
	assert.Error(t, err)
	assert.Equal(t, err.Error(), "canceling query due to user request")
}

func Test_GetByUuid_Success(t *testing.T) {
	userId := uuid.New()
	roleId := uuid.New()
	createdAt := time.Now()
	db, mock, err := sqlmock.New()
	...
	repo := PostgresRepository{db}
	createdUser, err := repo.GetByUUID(ctx, userId)
